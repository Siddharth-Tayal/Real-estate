...
export default function EditListing() {
  ...
  useEffect(() => {
    const listingElement = currentListing.filter(
      (item) => item._id === listingId
    );
    setFormData(listingElement[0]);
  }, [listingId]);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const result = await fetch(`/api/listing/update/${listingId}`, {
        method: "PUT",
        headers: {
      ...
      if (data.success === false) {
        dispatch(EditListingFalse(data.message));
        setFormError(data.message);
        setFormLoading(false);
        return;
      }
      dispatch(EditListingSuccess(data.listing));
      setFormLoading(false);
      setFormError(null);
      navigate(`/listing/${listingId}`);
    } catch (error) {
      dispatch(EditListingFalse());
      setFormLoading(false);
  ...
  return (
    <main className=" p-3 max-w-4xl mx-auto">
      <h1 className="font-semibold my-7 text-3xl text-center">Edit Listing</h1>
